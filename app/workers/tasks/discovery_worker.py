...
def deep_research_worker(self, campaign_id: str):
    ...
    try:
        try:
            ...
            # release before retry so the requeued attempt can re-acquire the lock
            release_lock(lock_key)
            self.retry(exc=e)
            return
        ...
        # Safety net for OOM-kills / unexpected exits; redundant after a clean
        # retry-return above (release_lock is idempotent).
        release_lock(lock_key)
